...
1. **Environment Variables**
   ...
   - Secure `CORS_ORIGINS` (remove wildcard)
...
- **Services:** Backend and frontend managed by supervisor
...
│   │   │   └── SKUDetail.js     # SKU detail workbench
...
│
├── scripts/
│   ├── create_mock_data.py      # Generate CSV mock data
│   └── load_data_to_mongodb.py  # Load CSV data to MongoDB
│
├── data/
│   ├── aggregated_data.csv      # Historical sales data
...
**SKUs:** 15 unique items (`CUST_003_ITEM_0001` to `CUST_003_ITEM_0015`)

**Historical Data:**
- **Duration:** 13 weeks (ending 2025-04-20)
- **Total Records:** 195 (15 SKUs × 13 weeks)
- **Fields:** item_id, timestamp, units_sold, demand_drivers (JSON)

**Forecast Data:**
- **Duration:** 40 weeks (starting 2025-04-27)
- **Total Records:** 15 (1 per SKU)
- **Forecast Points per SKU:** 40 weeks
- **Fields:** item_id, inference_date, forecasts (JSON array), demand_drivers (JSON array)
...
```python
# Get recent historical average (last 3 weeks)
avg_historical = sum(units_sold) / 3

# Get near-term forecast average (first 3 weeks)
avg_forecast = sum(forecast_means) / 3
...
if abs(deviation) > 40:
    ...
    # Not flagged as alert
    pass
```

### Time Range Logic

**Current Week:** Defined as `max(inference_date)` from `forecasts` collection (2025-04-20)

**Historical Window:** 13 weeks prior to current week

**Forecast Window:** 39 weeks following current week (weeks 1-40 from forecast array)

**Total View:** 52 weeks (13 + 39)
...
- **Solution:** Check browser console for Recharts errors. Ensure data is in correct format.

**Issue:** API returns 404
- **Solution:** Verify MongoDB has data loaded. Check SKU ID format.

**Issue:** Frontend can't connect to backend
- **Solution:** Check `REACT_APP_BACKEND_URL` in `.env`. Ensure backend is running on correct port.

**Issue:** MongoDB connection failed
- **Solution:** Verify MongoDB is running (`sudo systemctl status mongod`). Check `MONGO_URL` in backend `.env`.

**Issue:** Hot reload not working
- **Solution:** For `.env` changes, restart services:
  ```bash
...
- [Google Fonts - Chivo](https://fonts.google.com/specimen/Chivo)
...
- **Development Team**: Built for retail supply chain demand planning
- **Design System**: Swiss & High-Contrast aesthetic
- **Data Architecture**: MongoDB with normalized schema
...
### Planned Features
1. **Export Functionality**: Download forecast data as CSV/Excel
2. **User Preferences**: Customizable alert thresholds
3. **Comparison View**: Overlay multiple SKUs on single chart
4. **Forecast Confidence Score**: Visualize prediction reliability
5. **Historical Accuracy Tracking**: Compare previous forecasts vs. actuals
6. **Automated Email Alerts**: Send notifications for high-priority items
7. **Advanced Filters**: Filter by category, region, or other SKU attributes
8. **What-If Scenarios**: Simulate different demand driver values
9. **Mobile Responsive Design**: Optimize for tablet and mobile devices
